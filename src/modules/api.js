...
// 🚀 Copiá y pegá esta versión exacta en tu api.js:

export async function insertarPrestamoCabecera(usuarioId, observaciones) {
  const { data, error } = await supabaseClient
    .from("prestamos")
    .insert([
      {
        usuario_id: parseInt(usuarioId),
        observaciones: observaciones, // Acá entra el curso/ubicación limpio
      },
    ])
    .select();

  if (error) {
    console.error("❌ Error específico de Supabase en Cabecera:", error);
    throw error;
  }
  return data[0].id;
}

// 4. Registrar máquinas del lote en la tabla secundaria (Detalle)
// 🚀 Versión inteligente: procesa un solo ID o un Array completo de forma automática sin tocar app.js
export async function insertarPrestamoDetalle(
  prestamoId,
  equipoIdOrArray,
  horaCatedraNum,
) {
  // CASO A: Si el frontend le pasa un Array (un lote de netbooks), armamos el lote para Supabase
  if (Array.isArray(equipoIdOrArray)) {
    if (equipoIdOrArray.length === 0) return;

    const filasDetalle = equipoIdOrArray.map((equipoId) => ({
      prestamo_id: prestamoId,
      equipo_id: parseInt(equipoId),
      hora_catedra: parseInt(horaCatedraNum),
    }));

    const { error } = await supabaseClient
      .from("detalle_prestamos")
      .insert(filasDetalle); // ⚡ Un solo viaje para insertar el lote completo

    if (error) {
      console.error("❌ Error en inserción masiva de detalles:", error);
      throw error;
    }
  }
  // CASO B: Si recibe un solo ID común (como venía haciendo antes tu app.js), corre normal
  else {
    const { error } = await supabaseClient.from("detalle_prestamos").insert([
      {
        prestamo_id: prestamoId,
        equipo_id: parseInt(equipoIdOrArray),
        hora_catedra: parseInt(horaCatedraNum),
      },
    ]);

    if (error) {
      console.error("❌ Error en inserción individual de detalle:", error);
      throw error;
    }
  }
}

//5. Actualiza el estado de las máquinas (Admite ID único o un Array de IDs automáticamente)
// Actualiza el estado de las máquinas (Blindada contra strings vacíos o NaN)
export async function actualizarEstadoEquipo(equipoIdOrArray, nuevoEstado) {
    if (!equipoIdOrArray) return;

    let query = supabaseClient.from('equipos').update({ estado: nuevoEstado });

    // Si recibe una lista de netbooks
    if (Array.isArray(equipoIdOrArray)) {
        // 🧼 Filtramos y convertimos limpiando cualquier espacio vacío o valor roto (NaN)
        const idsNumericos = equipoIdOrArray
            .map(id => parseInt(id, 10))
            .filter(id => !isNaN(id)); // 👈 Chau dolores de cabeza con comas colgadas

        if (idsNumericos.length === 0) return;
        
        query = query.in('id', idsNumericos);
    } 
    // Si recibe un único ID
    else {
        const idUnico = parseInt(equipoIdOrArray, 10);
        if (isNaN(idUnico)) return;
        query = query.eq('id', idUnico);
    }

    const { data, error } = await query;
    if (error) {
        console.error("❌ Error real devuelto por Supabase en actualización de estado:", error);
        throw error;
    }
    return data;
}
...
export async function obtenerDisponibilidadEquipos(
  ...
  try {
    ...
    const idsOcupados = reservasOcupadas
      ? reservasOcupadas.map((r) => r.equipo_id)
      : [];
    ...
    console.error(
      "Error en obtenerDisponibilidadEquipos con horas:",
      error.message,
    );
    throw error;
  }
...
export async function guardarReservaMasiva(filas) {
  try {
    const { data, error } = await supabaseClient.from("reservas").insert(filas);
    if (error) throw error;
    return data;
  } catch (error) {
...
export async function eliminarReservasMasivas(idsArray) {
  ...
  if (error) {
    throw new Error(
      "No se pudieron eliminar las reservas de la base de datos: " +
        error.message,
    );
  }
  return data;
